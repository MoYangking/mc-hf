...
## 示例：新增一个 Web 终端（GoTTY）进程并通过 Nginx 暴露

本项目内置了 GoTTY 安装与 Supervisor 进程配置（见 Dockerfile 与 `supervisor/supervisord.conf`）。

- 构建阶段（Dockerfile）：
  - 在 `amd64` 架构下自动拉取二进制 `gotty_v${GOTTY_VERSION}_linux_amd64.tar.gz` 并解压到 `/home/user/gotty`。
  - 变量：`GOTTY_VERSION`（默认 `1.6.0`）。

- 运行阶段（Supervisor）：
  - 进程名：`gotty`；监听 `127.0.0.1:8080`；命令：`/home/user/gotty --address 127.0.0.1 --port 8080 --path /terminal -w /bin/bash`。
  - 配置位置：`supervisor/supervisord.conf`。

- 网关路由（Nginx）：
  - 默认新增两条规则（见 `nginx/default_admin_config.json`）：
    - `path_equal=/terminal` → 301 重定向到 `/terminal/`
    - `path_prefix=/terminal/` → 代理到 `http://127.0.0.1:8080`
  - 因为全局 location `/` 已开启 WebSocket 相关头，这个路由天然支持 GoTTY 的 WebSocket 通信。

构建完成后，可直接访问：`https://<host>/terminal/`

如需关闭或更改路由，可通过 `/admin/ui/` 或 `POST /admin/routes.json` 调整规则。
