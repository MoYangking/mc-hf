...
## 示例：新增一个 Web 终端（ttyd）进程并通过 Nginx 暴露

本项目内置了 ttyd 安装与 Supervisor 进程配置（见 Dockerfile 与 `supervisor/supervisord.conf`）。

- 构建阶段（Dockerfile）：
  - 自动根据架构拉取二进制（`amd64 → ttyd.x86_64`，`arm64 → ttyd.aarch64`）并放到 `/home/user/ttyd`。
  - 变量：`TTYD_VERSION`（默认 `1.7.7`）。

- 运行阶段（Supervisor）：
  - 进程名：`ttyd`；监听 `127.0.0.1:7681`；命令：`/home/user/ttyd -p 7681 bash`。
...
- 网关路由（Nginx）：
  ...
  - 因为全局 location `/` 已开启 WebSocket 相关头，这个路由天然支持 ttyd 的 WebSocket 通信。
